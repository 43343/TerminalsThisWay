...
{
    isDialogOpen = true; // Устанавливаем флаг, что диалог открыт
    ...
    bi.ulFlags = BIF_RETURNONLYFSDIRS | BIF_NEWDIALOGSTYLE; // Используем новый стиль диалога
    ...
    isDialogOpen = false; // Сбрасываем флаг, так как диалог закрыт
        ...
        else
        {
                ...
                if ((wParam == WM_KEYUP || wParam == WM_SYSKEYUP) && pKeyboard->vkCode == ConfigManager::getInstance().getConfig().keyChooseFolder1)
                {
                    ...
                    if (!g_ChooseFolderInstance->firstPressDetected) {
                        ...
                        if (duration <= DOUBLE_PRESS_INTERVAL_MS) {
                            ...
                            firstPressTimeChooseFolder = currentTime; // Обновляем время первого нажатияvv
                        }
                    }
                }
            }
            else
            {
                if (wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN) {
                    if (pKeyboard->vkCode == ConfigManager::getInstance().getConfig().keyChooseFolder1) { // Код для 'A'
                        g_ChooseFolderInstance->keyA_pressed = true;
                    }
                    if (pKeyboard->vkCode == ConfigManager::getInstance().getConfig().keyChooseFolder2) { // Код для 'B'
