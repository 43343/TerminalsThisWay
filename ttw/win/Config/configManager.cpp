...
{
	std::wifstream infile{ fs::path(filePath) };
	if (infile.is_open()) {
	...
	std::wofstream outfile{ fs::path(filePath) };
	if (outfile.is_open()) {
		outfile << L"PathToTerminal=C:\\WINDOWS\\system32\\cmd.exe\n";
		outfile << L"LaunchByDefault=0\n";
		outfile << L"RunAsAdministrator=0\n";
